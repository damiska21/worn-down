...
func _physics_process(delta: float) -> void:
	...
	var attacking = get_node("Attack").attacking
	# Handle jump.
	if Input.is_action_just_pressed("move_up") and is_on_floor() and !attacking:
	...
	if direction and !attacking: # při útočení se nemůžeš hýbat
